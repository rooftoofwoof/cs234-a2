...
class Linear(DQN):
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        updates = []
        get_c = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=q_scope)
        get_target_c = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
        for i in range(len(get_c)):
            updates.append(tf.assign(get_target_c[i], get_c[i])) 
        self.update_target_op = tf.group(*updates)
        ##############################################################
        ######################## END YOUR CODE #######################
